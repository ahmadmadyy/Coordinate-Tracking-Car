...
void setup() {
  ...
  // Create tasks
  xTaskCreate(keypadTaskFunction, "KeypadTask", 1000, NULL, 2, &keypadTask);
  xTaskCreate(encoderTaskFunction, "EncoderTask", 1000, NULL, 2, &encoderTask);
  xTaskCreate(motorControlTaskFunction, "MotorControlTask", 1000, NULL, 2, &motorControlTask);
...
void loop() {
  // Empty loop, the scheduler will take care of running tasks
}

void keypadTaskFunction(void* pvParameters) {
  // Handle keypad input here
  while (1) {
    // Read keypad and handle input
...
{ 
  ...
  while (1) {
    // Implement your motor control logic
    vTaskDelay(pdMS_TO_TICKS(100)); // Adjust the delay as needed
  }
}
